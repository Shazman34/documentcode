...
class SaleOrder(Model):
    ...
    def get_lazada_order(self,acc_id,order,context={}):
        ...
        if not acc:
            raise Exception("Lazada Account not Found for ID: %s" % acc_id)
        if not order:
        ...
        for it in order["item_list"]:
            ...
            res=get_model("sync.record").search([["sync_id","=",str(it["item_id"])],["related_id","like","product"],["account_id","=","lazada.account,%s"%acc_id]]) # XXX
            if not res:
