...
class Picking(Model):
    ...
    """
    def get_lazada_order(self,acc_id,order,context={}):
        ...
        pick_vals={
            "number": "Lazada - %s" % order["order_sn"],
            "type": "out",
        ...
        for it in order["item_list"]:
            res=get_model("sync.record").search([["sync_id","=",str(it["item_id"])],["related_id","like","product"],["account_id","=","lazada.account,%s"%acc_id]])
            if not res:
        ...
        picks = get_model("stock.picking").search_browse([["number","=","Lazada - %s" % order["order_sn"]]])
        pick = None
        if pick_sync:
