...
class ProductCateg(Model):
    ...
    _fields = {
        "is_lazada": fields.Boolean("Lazada Category",search=True),
    ...
    def get_lazada_brands(self,ids,acc_id=None,context={}):
        #obj=self.browse(ids[0])
        for obj in self.browse(ids):
            if not acc_id:
                res=get_model("lazada.account").search([])
                if not res:
                    raise Exception("Lazada account not found")
                acc_id=res[0]
            if not obj.sync_id:
                raise Exception("Missing lazada ID")
            ...
            for r in resp["brand_list"]:
                vals={
                    "name": "Lazada - "+r["original_brand_name"],
                    "categs": [("set",[obj.id])],
                    "sync_records": [("create",{
                        "sync_id": r["brand_id"],
