...
class LazadaProductCategory(Model):
    _name = "lazada.product.categ"
    _string = "Lazada Product Category"
    _fields = {
        "sync_id": fields.Char("Sync ID"), #XXX category_id
        "parent_id": fields.Many2One("lazada.product.categ","Parent Category",search=True), #parent_category_id = parent_id.sync_id
    ...
    def name_get(self, ids, context={}):
        print("lazada.product.categ.name_get",ids)
        vals = []
    ...
    def get_categ(self,context={}):
        print("lazada.product.categ.get_categ")
        if context.get("account_id"):
            acc = get_model("lazada.account").browse(context["account_id"])
        else:
            acc = get_model("lazada.account").search_browse([])[0]
        ...
        for r in resp["category_list"]:           
            ...
            if r["parent_category_id"]:
                parents = get_model("lazada.product.categ").search([["sync_id","=",str(r["parent_category_id"])]])
            ...
            #print("vals",vals)
            categs = get_model("lazada.product.categ").search([["sync_id","=",str(r["category_id"])]])
            if len(categs) == 0:
                get_model("lazada.product.categ").create(vals)
            else:
                get_model("lazada.product.categ").write(categs,vals)
            db.commit()
            i+=1

LazadaProductCategory.register()
