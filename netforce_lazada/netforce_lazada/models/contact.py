...
class Contact(Model):
    ...
    def get_lazada_contact(self, acc_id, order, context={}):
        if not acc_id:
            raise Exception("Missing Lazada acc_id")
        acc = get_model("lazada.account").browse(acc_id)
        if not acc:
            raise Exception("Lazada Account not found with ID %s" % acc_id)
        if not order:
        ...
        cont_vals={
            ...
            "sync_records": [("create",{
                ...
                "account_id": "lazada.account,%s"%acc.id,
            })],
        }
        addr_vals={
        ...
        contact_sr_ids = get_model("sync.record").search_browse([["related_id","like","contact"],["sync_id","=",str(order["buyer_user_id"])],["account_id","=","lazada.account,%s"%acc_id]])
