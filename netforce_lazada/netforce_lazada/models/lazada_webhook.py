...
class Webhook(Model):
    _name = "lazada.webhook"
    _string = "Lazada Webhook"
    _fields = {
    ...
    def handle_webhook(self,ids,context={}):
        ...
        for obj in self.browse(ids):
            ...
            try:
                ...
                res = get_model("lazada.account").search_browse([["shop_idno","=",str(shop_id)]])
                if res.company_id:
                    access.set_active_company(company_id.id)
                if not res:
                    raise Exception("Lazada Account not found with ID %s"%shop_id)
                acc = res[0]
                if code == 0:
                    pass
                elif code ==  3:
                    ...
                    if order_id and data["status"]:
                        order = get_model("lazada.order").browse(order_id)
                    ...
                    order_id = acc.get_order(order_sn)
                    order = get_model("lazada.order").browse(order_id)
                    # orders = get_model("lazada.order").search_browse([["order_sn","=",order_sn]])
                    if order:
                        # order = orders[0]
                        order.write({
                        ...
                        if order.pickings:
                            ...
                            logs = order.logs or ""
                            log = "%s :lazada.webhook.handle_webhook\n" % datetime.now().strftime("%Y-%m-%d %H:%M:%S")
