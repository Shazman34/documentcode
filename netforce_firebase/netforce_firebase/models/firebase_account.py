...
class Account(Model):
    ...
    def import_vals(self,model,data):
        ...
        for n,v in data.items():
            ...
            if isinstance(f,fields.Many2One) and v:
                ...
                res=rm.search([["sync_id","=",str(v)]])
                if not res:
                    res=rm.search([["id","=",int(v)]]) # XXX
                    if not res:
    ...
    def process_event(self,msg,context={}):
        print("process_event",msg)
        access.set_active_user(1)
        access.set_active_company(5) # XXX: sanko
        ...
        if event_type=="put":
            if data is not None:
                ...
                obj_id=res[0]
                get_model(model).delete([obj_id])
        elif event_type=="patch": # XXX
            record_id=int(sync_id) # XXX
            print("record_id",record_id)
            res=get_model(model).search([["id","=",record_id]])
            if not res:
                raise Exception("Record not found: %s %s"%(model,sync_id))
            obj_id=res[0]
            vals=self.import_vals(model,data)
