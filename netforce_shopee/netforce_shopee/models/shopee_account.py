...
class Account(Model):
    ...
    _fields = {
        ...
        "invoice_prefix": fields.Char("Invoice Prefix"),
        "payment_prefix": fields.Char("Payment Prefix"),
        "pick_out_prefix": fields.Char("Goods Issue Prefix"),
        ...
        "shopee_charged_shipping_fee_account_id": fields.Many2One("account.account","Shopee Charged Shipping Fee Account"),
        "ewallet_account_id": fields.Many2One("account.account","Shopee E-Wallet Account"),
    ...
    def get_orders(self,ids,context={}):
        # print("shopee.account.get_orders",ids)
        order_ids = []
        for obj in self.browse(ids):
            ...
            job_id = context.get("job_id", None)
            i = 0
            for order_no in order_nos:
                ...
                order_id = self.get_order([obj.id],order_no)
        ...
        return {
            "next": {
                "name": "shopee_order",
                "mode": "list",
                "search_condition": '[["id","in",%s]]' % order_ids,
                "active_tab": "9",
            },
            "flash": "%s Orders Updated." % len(order_ids),
        }
    ...
    def get_payments(self,ids,context={}):
        ...
        for obj in self.browse(ids):
            ...
            url = self.generate_url(account_id=obj.id, path=path)
            last_update_time = datetime.strptime(obj.payment_last_update_time,"%Y-%m-%d %H:%M:%S")
            if not last_update_time:
                raise Exception("Missing Shopee Payment Last Update Time for %s. Please Select a Time in Shopee Account." % obj.name)
            # print("Last Update Time: %s" % last_update_time)
            order_nos = []
            page_no = 1
            current_time = datetime.now()
            while len(order_nos) < 10000:
                time_to = min(last_update_time + timedelta(days=15),current_time)
                url2 = url + "&release_time_from=%s"%int(last_update_time.timestamp())
                url2 += "&release_time_to=%s"%int(time_to.timestamp())
                url2 += "&page_size=%s" % (context.get("page_size") or "100")
                url2 += "&page_no=%s" % page_no
                # print("url2",url2)
                req=requests.get(url2)
                res=req.json()
                if res.get("error"):
                ...
                # print("res",res)
                resp=res["response"]
                for r in resp["escrow_list"]:
                    order_nos.append({
                        "order_sn": r["order_sn"],
                        "payout_amount": r["payout_amount"],
                        "escrow_release_time": datetime.fromtimestamp(r["escrow_release_time"]).strftime("%Y-%m-%d %H:%M:%S"),
                        })
                if resp["more"]:
                    page_no += 1
                elif time_to == current_time:
            ...
            for es in order_nos:
                if job_id:
                    if tasks.is_aborted(job_id):
                        return
                    tasks.set_progress(job_id,i/len(order_nos)*100,"Downloading Payments: %s of %s."%(i,len(order_nos)))
                res = get_model("shopee.order").search_browse([["account_id","=",obj.id],["order_sn","=",es["order_sn"]]])
                if not res:
                    raise Exception("Shopee Order not Found: %s" % es["order_sn"])
                order = res[0]
                order.get_escrow_detail(context={"skip_error":True})
                if not order.invoices:
                    order.copy_to_invoice(context={"skip_error":True})
                order.write({"escrow_release_time":es["escrow_release_time"]})
                order.pay_invoice(context={"skip_error":True})
                i += 1
            obj.write({"payment_last_update_time":time_to.strftime("%Y-%m-%d %H:%M:%S")})
